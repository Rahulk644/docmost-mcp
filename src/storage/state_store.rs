...
impl StateStore {
    pub fn new(base_dir: Option<PathBuf>) -> Result<Self> {
        ...
        Ok(Self {
            ...
            base_dir,
            keyring: KeyringStore,
            allow_file_credentials: explicit_base_dir
                || matches!(
                    std::env::var("DOCMOST_ALLOW_FILE_CREDENTIALS")
                        .ok()
                        .as_deref(),
                    Some("1") | Some("true")
                ),
    ...
    pub async fn read_credentials(&self) -> Result<Option<StoredCredentials>> {
        ...
        if !self.allow_file_credentials {
            return Ok(None);
        }
    ...
    pub async fn write_credentials(&self, credentials: &StoredCredentials) -> Result<()> {
        ...
        if !self.allow_file_credentials {
            tracing::warn!(
                "System keyring unavailable; credentials were not written to disk. Set DOCMOST_ALLOW_FILE_CREDENTIALS=true to opt in to encrypted-file fallback."
            );
            return Ok(());
        }
